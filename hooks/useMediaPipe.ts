  ...
  useEffect(() => {
    ...
    const processResults = (results: HandLandmarkerResult) => {
        ...
        if (results.landmarks) {
          handPositionsRef.current.isLeftFist = false;
          handPositionsRef.current.isRightFist = false;
          for (let i = 0; i < results.landmarks.length; i++) {
            ...
            let foldedFingers = 0;
            for (let j = 0; j < 4; j++) {
              const tipDist = Math.hypot(landmarks[fingerTips[j]].x - wrist.x, landmarks[fingerTips[j]].y - wrist.y);
              const knuckleDist = Math.hypot(landmarks[fingerKnuckles[j]].x - wrist.x, landmarks[fingerKnuckles[j]].y - wrist.y);
              if (tipDist < knuckleDist) foldedFingers++;
            }
            const isFist = foldedFingers >= 3;

            if (isRight) {
                  newRight = worldPos; 
                  handPositionsRef.current.isRightFist = isFist;
            } else {
                  newLeft = worldPos;
                  handPositionsRef.current.isLeftFist = isFist;
            }
          }
        } else {
          handPositionsRef.current.isLeftFist = false;
